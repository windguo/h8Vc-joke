...
export default class User extends Component {
    ...
    loadData = async(resolve)=>{
        let url = urlConfig.userInfo + '&userid=' + this.props.navigation.state.params.userid;
    ...
    loadMore = async()=>{
        this.requestPageNumber += 1;
        let url = urlConfig.userInfo + '&userid=' + this.props.navigation.state.params.userid + this.dealWithrequestPage();
        let res = await HttpUtil.GET(url);
        if(!res||!res.result){
            return;
        }
